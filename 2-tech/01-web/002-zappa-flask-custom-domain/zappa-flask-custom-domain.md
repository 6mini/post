...
[자파(Zappa)를 이용한 플라스크(Flask) 서버리스 애플리케이션의 AWS 람다(Lambda) 배포를 진행했던 지난 포스팅](/zappa-flask-serverless-lambda)에 이어 이번 포스팅에서는 사용자 정의 도메인 연결 과정을 다룰 것이다.
...
- 이전 아티클: [Zappa를 통한 Flask 서버리스 배포(1): AWS Lambda에 배포](/zappa-flask-serverless-lambda)
